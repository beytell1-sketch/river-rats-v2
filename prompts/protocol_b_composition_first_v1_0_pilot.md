...
derived_from: prompts/protocol_b_composition_first_v1_0.md @ master c4f29a5 (Protocol B v1.0.1 design artifact)
artifact_class: PILOT-RUNTIME (labeller-facing; self-contained)
status: v1.0.1-pilot (Build A artifact per MAIN_TERMINAL_PILOT_HALT_ACK_BUILDS_ABC_DIRECTIVE_2026-04-26.md `3f9564e`); inheritance-by-reference paragraphs in §Buckets, §Features, §DO NOT Rules replaced with verbatim-inlined v3.1 content
review_chain:
  ...
  - v1.0.1 fix-forward — addresses 3 MEDIUM + 2 NIT findings — sealed via PR #X (Protocol B Wave 2)
  - v1.0.1 design artifact canonical at master `c4f29a5`
  - v1.0.1-pilot Build A pass (this artifact, 2026-04-26) — verbatim-inline of v3.1 §Bucket taxonomy (lines 170-204), §Features (lines 439-496) + 4 v2.4 P1 blocker features, §DO NOT Rules (lines 595-647); see §"Pilot artifact build provenance" below
  - v1.0.1-pilot independent reviewer pass — REQUIRED before pilot dispatch
  - PRE-DISPATCH PREREQUISITES row #5 GREEN — REQUIRED before Phase A.1
build_provenance:
  source_design_artifact: prompts/protocol_b_composition_first_v1_0.md
  ...
  inlined_sections:
    ...
    - "§DO NOT Rules: v3.1 lines 590-647 (Rules 1-11)"
...
## Pilot artifact build provenance (Build A output)

**THIS FILE IS THE LABELLER-FACING PILOT ARTIFACT.** Self-contained.
No external file lookups required during labelling.

This file was produced by the Build A step per the Protocol B v1.0.1
...
Source: `prompts/protocol_b_composition_first_v1_0.md` (design artifact)
at master `c4f29a5`. The following sections in the design artifact
used inheritance-by-reference; this pilot artifact replaces those
references with verbatim-inlined content from
...
- §"DO NOT Rules" — verbatim from v3.1 §"DO NOT Rules" (lines 590-647)
...
PRE-DISPATCH PREREQUISITES row #4 pattern, applied to Protocol B
analogously to Protocol A v3.1 frozen + checksum).

**Out of labeller scope:** sections in this file referencing reviewer
process (§Calibration, §Author note, §Reference, frontmatter) are
not labeller instructions; the labeller's working scope is
§"Reasoning Order" through §"Anti-patterns".
...
Protocol B's distinction is REASONING ORDER, not bucket definitions.
...
Rules at lines 595-647; the design artifact's v1.0.1 summary listed
...
## Output schema (verbatim-inlined v3.1 fields + Protocol-B additions)
...
v3.1 fields), plus the Protocol B additions:
...
```json
{
  ...
  // ---- Protocol B additions below (NOT in v3.1) ----
  "protocol": "B",
  "composition_derived_candidates": ["BET", "RAISE"],
  "bucket_aligned_action": "RAISE",
  "outcome_4a_or_4b": "4A",
  "composition_rule_conflict": false,
  "composition_reasoning_trace": "Villain comp: 0.30 TP+ / 0.15 medium / 0.40 draws / 0.15 air. Heavy-draws + nut-draw hero with blocker → semi-bluff RAISE composition-derived (Step 3). Bucket taxonomy: drawing + KB 1.7 anchor → RAISE. Match (Outcome 4A).",
...
strip the comment line at output time. The `composition_reasoning_trace`
is replaced per-hand with the actual TP+/medium/draws/air composition
percentages and reasoning derived in Step 1-3.)
