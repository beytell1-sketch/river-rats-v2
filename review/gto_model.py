...
class GtoOracle:
    ...
    def __init__(self, model_path: str):
        ...
        # Auto-detect class count: 5-class (HU) or 3-class (multiway specialist)
        n_cls = self._model.n_classes_
        if n_cls == N_CLASSES:
            self._action_map = INT_TO_ACTION
            self._action_names = ACTION_CLASSES
        elif n_cls == 3:
            # 3-class multiway: 0=FOLD, 1=CALL, 2=RAISE
            _3WAY_CLASSES = ("FOLD", "CALL", "RAISE")
            self._action_map = {i: a for i, a in enumerate(_3WAY_CLASSES)}
            self._action_names = _3WAY_CLASSES
        else:
            raise ValueError(
                f"Model has {n_cls} classes, expected {N_CLASSES} or 3"
            )
