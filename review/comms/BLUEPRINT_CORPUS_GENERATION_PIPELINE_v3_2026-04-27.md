---
date: 2026-04-27
from: architect (Phase 2.6 mechanical-corrections subagent)
to: orchestrator → owner → lead-programmer
re: Blueprint v3 — 7 mechanical corrections applied (C1-C7); supersedes v2
supersedes: BLUEPRINT_CORPUS_GENERATION_PIPELINE_v2_2026-04-27.md (PR #56 closed)
status: BLUEPRINT v3 — corrections applied per round-2 synthesis (master 0f83f29)
---

# Architect Phase 2.6 — corpus-generation pipeline blueprint v3

## What changed in this update

This document supersedes blueprint v2 from PR #56 (`21b99fe`). Blueprint v2 received APPROVE-WITH-NITS from ml-architect and gto-expert (round 2), and APPROVE clean from QC. This v3 is a mechanical correction pass applying the 7 corrections required by the round-2 synthesis (master `0f83f29`). No redesign; only targeted corrections.

**Changes applied (v2 → v3):**

| ID | Change | Section |
|----|--------|---------|
| C1 | R5 Pair 5 board corrected: `JsTd4d` (unpaired, mislabelled "draw-heavy paired") → `JsJd9c` (paired-J + 9, genuinely draw-heavy paired) | Q2 Rule 11 boundary table |
| C2 | R5 Pair 4 board corrected: `9h6h3h` (monotone, outside Rule 11 scope) → `9d6d3s` (2-tone-flush, within Rule 11 "paired / 2-tone-flush" scope) | Q2 Rule 11 boundary table |
| C3 | Module 8 hero position specified per sub-scenario (5 sub-scenarios explicit: 8a–8e with CO/BTN assignments and `num_callers_to_bet` documented) | Q6 Module 8 spec |
| C4 | R1 re-extraction algorithm: 3-line edge case handler added for 3-bet pots (SB `is_3bet_pot=1` guard prevents incorrect IS_PFA=1 on 3 affected hands) | Q6 re-extraction algorithm |
| C5 | R2 warm-start base model reference corrected: "v8 oracle" → `gto_model_v9_baseline_45feat.json` (actual v9 baseline artifact) | Q6 R2 script spec + Step 5 test plan |
| C6 | R2 failure path replaced: "STOP and report BLOCKED" → explicit feature-mismatch resolution path (45-vs-59 mismatch is expected; documented handling) | Q6 R2 script spec |
| C7 | N1 smoke test assertion field name corrected: `pot_chips > 60` → `pot_bb > 6.0` (converted record uses `pot_bb`, not `pot_chips`) | Q6 Step 1 test plan + nits section |
...
- All 5 R-items + 2 scope additions (Modules 8/9) stand from v2
- All non-blocking nits from v2 (N1/N2/N3 already incorporated; gto-expert N4/N5/Pattern D/MAGG calibration timing folded into programmer phase — see end of document)

---

## Sources read for this update

All source files read directly (Phase 2.6 architect):

- `review/comms/MAIN_TERMINAL_BLUEPRINT_v2_REVIEW_SYNTHESIS_2026-04-27.md` (master `0f83f29`) — synthesis dictating the 7 corrections
- `review/comms/REVIEW_ML_ARCHITECT_BLUEPRINT_v2_PR56_2026-04-27.md` — round-2 ml-architect review (C4, C5, C6, C7 source)
- `review/comms/REVIEW_GTO_EXPERT_BLUEPRINT_v2_PR56_2026-04-27.md` — round-2 gto-expert review (C1, C2, C3 source)
- Blueprint v2 under correction: `git show orch/blueprint-v2-corpus-generation-2026-04-27:review/comms/BLUEPRINT_CORPUS_GENERATION_PIPELINE_v2_2026-04-27.md` (PR #56 head `21b99fe`, 1016 lines)

*Sources from Phase 2.5 (carried forward, not re-read for this correction pass):*
...
| 4 | 2-tone-flush (corrected per C2) | 9d6d3s | 2-tone-flush low connected board — within Rule 11's "paired / 2-tone-flush" scope (replaces prior monotone 9h6h3h which was a 3-flush board outside Rule 11's scope). Villain's range at 0.40 TP+ threshold separates flush-draw combos with pair equity from offsuit air; structurally distinct from Pairs 1-3 (no pair, low connected, 2-tone) and from Pair 5 (which is paired). Exercises the 2-tone-flush branch of Rule 11. |
| 5 | Draw-heavy paired (corrected per C1) | JsJd9c | Paired-J board with 9 providing straight draw potential — genuinely paired and draw-heavy (replaces prior JsTd4d which was unpaired, mislabelled "draw-heavy paired"). Villain's range includes trips (Jx), sets of 9s, and some straight draws; TP+ threshold separates the value-dense trip/FH combos from the draw-heavy non-pair portion. GTO-meaningful for the 0.40 Rule 11 threshold because at villain_tp+_pct >= 0.40, the Jx and 9x combos form a strong calling range for OOP hero bets; below 0.40, villain's range is draw-heavy air that folds. |
...
**Correction notes (v3)**:
- Pair 4 (corrected per C2): `9d6d3s` — 2-tone-flush, low connected; replaces previous monotone `9h6h3h` which was a 3-flush board outside Rule 11's "paired / 2-tone-flush" scope. The new board exercises the 2-tone-flush branch of Rule 11 and is structurally distinct from all other 4 textures (unpaired, low, 2-tone).
- Pair 5 (corrected per C1): `JsJd9c` — paired-J draw-heavy board; replaces previous `JsTd4d` which was an unpaired connected two-tone board mislabelled "draw-heavy paired". The new board is genuinely paired and retains draw-heavy character via the 9 creating straight draw potential. It exercises the paired branch of Rule 11 at the villain_top_pair_plus_pct=0.40 threshold with a board composition (trips + 9-straight draws vs air) that produces the most informative signal for the Rule 11 boundary.
...
**Module 8: Donk-bet defence scenarios (hero position specified per C3)**

Hero is IP (BTN or CO), facing an OOP donk lead on the flop from the BB (non-PFA). Hero position is specified explicitly per sub-scenario because action order and `num_callers_to_bet` differ between CO-as-hero and BTN-as-hero.
...
**Sub-scenario specifications (corrected per C3) — hero position explicit per scenario:**

- **Sub-scenario 8a**: Hero CO, PFA = HJ. Action order: HJ open, CO call, BB defend; flop: BB donk, hero (CO) faces first; BTN is not in this hand (folded preflop or HJ opened and CO is next position with BB as 3rd). `num_callers_to_bet=0` at hero's decision (no one has called the donk when CO faces it). Hero acts before BTN if BTN is present; if BTN folded preflop this is HJ/CO/BB 3-way.
  - Action history: `[('preflop', 'HJ', 'raise'), ('preflop', 'CO', 'call'), ('preflop', 'BB', 'call'), ('flop', 'BB', 'bet')]`
  - `num_callers_to_bet=0` (hero CO acts first among IP players facing the donk)

- **Sub-scenario 8b**: Hero BTN, PFA = CO. Action order: CO open, BTN call, BB defend; flop: BB donk, CO acts first (calls or folds), then hero (BTN) faces. `num_callers_to_bet=0` if CO folds; `num_callers_to_bet=1` if CO calls before BTN.
  - Action history (CO folds): `[('preflop', 'CO', 'raise'), ('preflop', 'BTN', 'call'), ('preflop', 'BB', 'call'), ('flop', 'BB', 'bet'), ('flop', 'CO', 'fold')]` → hero (BTN) faces; `num_callers_to_bet=0`
  - Action history (CO calls): `[('preflop', 'CO', 'raise'), ('preflop', 'BTN', 'call'), ('preflop', 'BB', 'call'), ('flop', 'BB', 'bet'), ('flop', 'CO', 'call')]` → hero (BTN) faces; `num_callers_to_bet=1`

- **Sub-scenario 8c**: Hero CO, PFA = CO (hero is the PFA). Action order: CO raise, BTN call, BB call; flop: BB donk into CO (hero/PFA). Hero (CO) is IP to BB but is the preflop aggressor. `num_callers_to_bet=0`.
  - Action history: `[('preflop', 'CO', 'raise'), ('preflop', 'BTN', 'call'), ('preflop', 'BB', 'call'), ('flop', 'BB', 'bet')]`
  - `num_callers_to_bet=0` (hero CO acts before BTN on the donk); `is_preflop_aggressor=1` for hero

- **Sub-scenario 8d**: Hero BTN, PFA = BTN (hero is the PFA). Action order: BTN raise, SB fold, BB call; flop: BB donk into hero (BTN). Hero is IP and is the PFA. `num_callers_to_bet=0`.
  - Action history: `[('preflop', 'BTN', 'raise'), ('preflop', 'SB', 'fold'), ('preflop', 'BB', 'call'), ('flop', 'BB', 'bet')]`
  - `num_callers_to_bet=0` (heads-up after SB fold; hero BTN faces the donk); `is_preflop_aggressor=1`

- **Sub-scenario 8e**: Hero CO, PFA = CO (non-PFA villain also behind). Action order: CO raise, BTN call, BB call; flop: BB donk, hero (CO) is first to act with BTN still behind. Same as 8c structurally but with explicit BTN behind — this is the sandwich variant for hero CO. `num_callers_to_bet=0` at decision point but hero must account for BTN's potential raise.
  - Action history: same as 8c; the difference is in the scenario generator's `villain_positions` including BTN as a live player behind hero

**`num_callers_to_bet` summary by sub-scenario:**
| Sub-scenario | Hero | PFA | `num_callers_to_bet` | Notes |
|---|---|---|---|---|
| 8a | CO | HJ | 0 | CO acts first facing donk |
| 8b (CO folds) | BTN | CO | 0 | BTN faces donk after CO folds |
| 8b (CO calls) | BTN | CO | 1 | BTN faces donk after CO calls |
| 8c | CO | CO | 0 | Hero is PFA; BTN behind |
| 8d | BTN | BTN | 0 | HU vs BB after SB fold |
| 8e | CO | CO | 0 | Same as 8c; BTN live behind (sandwich variant) |
...
**`scripts/verify_feature_schema_compatibility.py`** — standalone verification script (corrected per C5 and C6):

- Load the warm-start base model artifact: **`river-rats-core/models/gto_model_v9_baseline_45feat.json`** (the actual v9 baseline — 45 features, trained on PokerBench, ready for warm-start; corrected per C5 from incorrect "v8 oracle" reference)
- Extract the model's feature column list (from `model.json` or the training artifact's `FEATURE_COLUMNS` metadata)
- Compare against the 59-feature corpus contract (derived from the actual new corpus record, not from `gto_model.py`'s current `FEATURE_COLUMNS` which has only 55 — the 4 v2.4 P1 blocker features are in `feature_keys.py` but not yet in `gto_model.py`)
- **Pass condition**: Exact match — same columns, same count, same order
- **Fail condition**: Any mismatch — script exits with non-zero status and prints specific column diffs (missing columns, extra columns, order differences)

**Usage (corrected per C5)**:
```bash
...
# Exit 1: FAIL — diff printed to stderr; see resolution path below
```

**Failure path resolution (corrected per C6 — replaces prior "STOP and report BLOCKED" instruction):**

The 45-vs-59 feature mismatch is EXPECTED and does not mean the corpus is wrong. The v9 baseline was trained on 45 features; the new corpus has 59 features (14 new features added since the v9 baseline). This is a known pre-condition, not an unexpected failure.

```
On feature mismatch:
- If new corpus has 59 features and base model has 45 features (expected):
    Document the 14 new features added since v9 baseline
    Train new trees with the 59-feature corpus
    Warm-start uses v9 base model's 45-feature trees as initial weights
    XGBoost handles the feature delta natively via xgb_model parameter
    This is the expected warm-start delta path; not a failure
- If unexpected mismatch (e.g. corpus has FEWER features than base model):
    Investigate; do not proceed with training
    Report to orchestrator with specific diff output before any further action
```

The programmer should run this check, note the expected 14-feature delta in the lock file, document the new feature names (the 14 features added since the v9-baseline 45-feature contract), and proceed with the warm-start training plan. Only an unexpected mismatch (corpus has fewer features than the base model, or features do not include all 45 base features as a subset) warrants STOP-and-report.
...
2. For each hand, reconstruct `_opener_position` from the hand's `prior_actions` (action history). The algorithm uses hero's own prior actions only (`prior_actions` stores hero's own preflop actions, not all players' actions). If hero's own `prior_actions` includes a preflop raise, hero IS the opener (`_opener_position = hero_position`); otherwise `_opener_position = None`.

   **Reconstruction algorithm (confirmed against live data — ml-architect verified across all 100 records):**
   ```
   ...
   **Edge case handler for 3-bet pots (corrected per C4)**: 3 records in the 100-hand corpus have `is_3bet_pot=1` (all SB position, pot=855 chips). In a 3-bet pot, the SB is the 3-bettor, not the original opener. The naive reconstruction (hero raised preflop → hero IS opener) incorrectly flags these 3 hands as `IS_PFA=1`. The programmer must add this guard:

   ```python
   # Edge case: 3-bet pots — SB position cannot be inferred as opener
   # from prior_actions alone (the 3-bettor may not be the original opener)
   if hand.get('feat_dict', {}).get('is_3bet_pot', 0) == 1:
       # In 3-bet pots, _opener_position is not reliably reconstructable
       # from prior_actions; default IS_PFA=0 for these hands
       _opener_pos = None
   ```

   This guard runs AFTER the preflop-raise detection and overrides `_opener_position` to None for any hand where `is_3bet_pot=1`. The `is_3bet_pot` field is available in the original `feat_dict` from the 100-hand JSONL. Three records affected (3%). Not blocking.
...
- **N1 (nit): Zero Mode A records with `spr < 2.0` AND `pot_bb > 6.0`** — regression test for unit-mismatch bug (corrected per C7: converted records use `pot_bb` not `pot_chips`; 6.0 BB = 60 chips / BB_CHIP_SIZE=10, equivalent threshold)
...
**Step 5: Feature schema compatibility check (corrected per C5 + C6)**
```bash
python3 scripts/verify_feature_schema_compatibility.py \
  --model river-rats-core/models/gto_model_v9_baseline_45feat.json \
  --feature-keys river-rats-core/feature_keys.py
```
Note: The expected result is exit 1 (FAIL) with a 14-feature delta (v9-baseline has 45 features; new corpus has 59). This is the expected warm-start delta path per C6's resolution — not a blocking failure. Document the 14 new feature names. Only an unexpected mismatch (corpus has fewer features than the base model) requires STOP-and-report. See Q6 R2 failure path resolution for handling.
...
### ml-architect nits (from round 2, already incorporated into v2)

- **N1 (ml-architect)**: SPR regression assertion in smoke test — Zero Mode A records with `spr < 2.0` AND `pot_bb > 6.0` (corrected per C7: `pot_bb > 6.0` is the correct field; `pot_chips` does not exist in converted records; 6.0 BB = 60 chips / BB_CHIP_SIZE=10). Catches unit-mismatch regression if the new generator miscalculates the BB conversion. 5 lines added to smoke test. *(Already incorporated into Step 1 test plan above.)*
...
### gto-expert non-blocking nits (from round 2 — programmer phase)

These gto-expert nits were non-blocking per the round-2 synthesis (master `0f83f29`). They are reproduced here for programmer reference and do not require further architectural decisions.

- **gto-expert N4 (non-blocking)**: Module 8 donk range description — the blueprint says BB's donk range is "polarised — either strong or air." In real GTO play the donk range also includes a substantial semi-bluff component (nut flush draws, combo draws on 2-flush boards) that cannot realize equity by checking. When Module 8 scenarios are generated, the programmer should add "or strong semi-bluffs (nut flush draws, combo draws on 2-flush boards)" to any labeller-facing donk range description so labellers reason correctly about the full donk range composition, not just value and air poles.

- **gto-expert N5 (non-blocking)**: Module 9 soft assertion — FOLD rate in Module 9 labelled hands should exceed 30%. SB's tighter MDF (~20% vs BB's ~33%) means SB folds more than any other facing-bet module. If the labelled Module 9 hands show FOLD < 30%, investigate whether the scenario specs are generating hands that are too strong for the SB position. Add as a post-labelling verification check.

- **gto-expert Pattern D escalation (non-blocking)**: Pattern D (blocker-driven bluff/fold, KB §1.10) escalates from LOW to LOW-MEDIUM severity in the context of Module 8. When Module 8 scenarios are generated on 2-flush boards, include at least 5 hands where hero explicitly holds a card in the flush suit (creating `flush_draw_block_pct > 0`). This provides Pattern D corpus coverage as a byproduct of Module 8 without a new module. Specify in `donk_bet_defence_scenarios.py`: 5+ hands where hero_cards include one card of the flush suit on boards with 2 of that suit.

- **gto-expert MAGG calibration timing (non-blocking)**: The OQ-3 handoff note for MAGG FOLD calibration hands should be read as: MAGG FOLD calibration hands (1-2) must be added to Tier 1 BEFORE or simultaneously with the first Tier 2 labelling batch that includes MAGG scenarios. Not merely "as part of Tier 1 expansion" generically — the timing dependency is explicit: labellers should see the MAGG FOLD calibration pattern before encountering 20+ MAGG hands in the main corpus.

- **gto-expert recommendation (from round 2)**: +1-2 MAGG FOLD calibration hands as part of Tier 1 expansion. This is a recommendation to the Tier 1 architect, not a change to this corpus-generation blueprint. *(Noted in OQ-3 handoff.)*

---

*Blueprint v3 complete. 7 mechanical corrections applied (C1-C7). No code written, no files modified except this document.*
*Next step: orchestrator verifies v3 (confirms all 7 corrections present and correctly applied) then merges. Lead-programmer dispatches after merge.*
