...
## Headline numbers (REVISED 2026-05-25 02:00 UTC after Opus retry correction)

**CRITICAL CORRECTION:** First Opus tier-up output had 4 solver-citation violations + 6 substantive poker errors (notably AA-on-coordinated-board mislabeled FOLD when canonical is RAISE for protection). Retry Opus reviewed and overwrote with corrected labels. Headline numbers reflect the CORRECTED consensus.

| | |
|---|---|
| Total spots | 50 |
| Consensus achieved | 47/50 (94%) |
| Owner-arb queue | 3 spots (Opus tier-up correction added 1; net same direction) |
...
## Consensus state breakdown (revised)
...
| 3-2+opus-disagree | 3 (→ owner-arb) |

## Action distribution (consensus_v2 modal, revised)
...
## Owner-arb queue (3 spots, REVISED after Opus retry correction)

### 4WF-CHAIN-009-004
- Sonnet: 3 CALL / 2 FOLD
- Opus (retry): FOLD (joins minority)
- 4-way SRP flop facing c-bet — genuinely close

### 4WF-CHAIN-009-016 (NEW from retry Opus)
- Sonnet: 3 FOLD / 2 CALL
- Opus (retry): CALL (joins minority)
- BB AsKs on Jc7s5d2h turn facing raise after own bet. NFD + 2 overcards + As-blocker. Retry Opus: CALL above FOLD by thin margin via implied equity on nut-flush rivers.

### 4WF-RANGE-AS-457 (NEW from retry Opus)
- Sonnet: 3 CALL / 2 RAISE
- Opus (retry): RAISE (joins minority)
- IP nut-gutshot + overcards. Retry Opus chose value/protection RAISE over CALL mix.

**Opus correction note:** First Opus tier-up output had 4 solver-citation violations + 6 substantive poker errors (notably AA-on-coordinated-board mislabelled FOLD). Retry Opus reviewed and overwrote. Net owner-arb change: -1 (4WF-RANGE-AS-452, where retry Opus joined Sonnet majority) + 2 (CHAIN-009-016, RANGE-AS-457) = +1.
