...
**Reasoning:** Hero holds the NUT flush (As-Ks-9s-6s-4s). The As is on the board, so no opponent can hold a higher flush — Ks is the highest possible spade in any player's hand. Hero is beaten only by full houses (44 = 3 combos, 99 = 3 combos, A4/A9 two pair improving = very rare given the monotone board discourages non-spade holdings from continuing). With a near-invincible hand, hero's equity against both opponents is extremely high (~80-85%). BTN bet small (33%) and CO called — both likely hold one-spade draws (e.g. Qs/Js with a side card), top pair Ax with no spade, or sets hoping to fill up. OOP position makes raising superior to calling: (a) hero cannot extract value efficiently on later streets from OOP if a fourth spade hits and opponents shut down, (b) building the pot NOW with the nuts maximises EV, and (c) opponents with flush draws will call a raise getting 3:1 or better, paying off hero's made nuts. Raise to ~2.5x the bet (~75-80) for value.

**Solver verification needed:** YES — RAISE with the nut flush on a monotone flop; confirm raise sizing and frequency vs call/slowplay.
...
**Reasoning:** Hero has the nut Broadway straight: A(board)-K(board)-Q(hand)-J(board)-T(hand) = AKQJT. This is the best possible hand. BTN's delayed c-bet (checked flop, bet turn after the Ks) suggests BTN picked up equity on the K turn — likely AK two pair, KK set, or KQ/KJ one-pair hands. Hero should call rather than raise because: (a) the nut straight benefits from deception — a raise folds out BTN's bluffs and weaker value hands that would pay off on the river, (b) hero closes action so there is no squeeze risk from behind, and (c) by calling hero keeps BTN's full barreling range alive for river value extraction. Raising is defensible (build the pot with the nuts) but the solver typically mixes here, with calling being the majority play when closing action with no players behind.

**Solver verification needed:** YES — CALL with the nuts (slowplay line); need to verify call vs raise frequency.

**Cross-reference note:** FB-17 (Agent 2) has the same nut straight on the same board but labels RAISE from a sandwich position. The solver typically mixes RAISE/CALL with the nuts. Both labels are acceptable — model scoring should accept either CALL or RAISE on FB-17 and FB-37.
