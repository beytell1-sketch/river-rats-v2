...
class PokerGame:
    ...
    def deal_hand(self, deck_override=None):
        ...
        self.street_actions: Dict[str, list] = {}  # {street: [(name, position, action), ...]}
    ...
    def _apply_action(self, player: Player, action: str, amount: int):
        ...
        # Record action for multi-street history (used by bridge for oracle features)
        if self.street not in self.street_actions:
            self.street_actions[self.street] = []
        self.street_actions[self.street].append((player.name, player.position, action))
    ...
    def deal_flop(self):
        ...
        self.street = 'flop'
        self._reset_street()
        if self.hero_callback is None and not self.headless:
    ...
    def showdown(self):
        ...
        _print = print if (self.hero_callback is None and not self.headless) else (lambda *a, **k: None)
