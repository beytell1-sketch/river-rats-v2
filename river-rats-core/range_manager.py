...
class RangeManager:
    ...
    def get_defend_range(self, hero_pos: str, vs_position: str) -> Dict[str, float]:
        ...
        Dynamically combines THREEB and CALL dicts. Takes the max
        frequency for each hand across both dicts.
        """
        # Get both ranges via existing methods
        threeb = self.get_3bet_range(hero_pos, vs_position)
        call = self.get_call_range(hero_pos, vs_position)
        
        # Combine: union of both dicts, max frequency per hand
        combined = dict(call)  # start with call range
        for hand, freq in threeb.items():
            combined[hand] = max(combined.get(hand, 0.0), freq)
    ...
    def get_call_range(self, hero_pos: str, vs_position: str) -> Dict[str, float]:
        ...
        # Get range
        if hero not in CALL:
            return {}  # SB, CO, HJ have no calling range (3-bet-or-fold)
