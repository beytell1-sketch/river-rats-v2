...
def _make_oracle_callback(oracle, params: dict,
    ...
    def callback(game, player, context):
        ...
        if action in ('fold', 'check'):
            ...
            # Oracle outputs 'raise' or 'bet' — map to correct game action.
            # 'bet' = first aggressive action on street (current_bet == 0).
            # 'raise' = increasing over an existing bet (current_bet > 0).
            amount = player.bet_this_street + max(int(context['pot'] * 0.67), 10)
            amount = min(amount, player.stack + player.bet_this_street)
            game_action = 'raise' if game.current_bet > 0 else 'bet'
...
class SelfPlayRunner:
    ...
    def run_round(self, round_id: int = 1) -> RoundResult:
        """Run one complete round: all deals × positions × variants.

        When single_position is set, only one hero position is used per deal.
        Combined with log_all_multiway, this avoids redundant hero rotations
        since all players' decisions are already captured in a single game.
        """
        ...
        positions = [self.single_position] if self.single_position else POSITIONS
        for deal in deals:
